...
//---------------------------------------------------------------------------
// инкремент счётчиков Com при ошибках
void ComTimers()
{
    for(int i=0;i<PORT_COUNT;i++ )
    {
        if(Comport[i]->port_err) Comport[i]->port_ct++;
    }
}
//---------------------------------------------------------------------------
...
void SComport::ComPanGen(unsigned char port_num)
{
	...
	LBL_otl->Top = 6;
	LBL_otl->Left = 150 + 115*(port_num);
	LBL_otl->Caption = "Отладка " + PortName;
...
void Init_SComport()
{
    ...
    Comport[3]->Timer_Int = 250;
    Comport[3]->Dev_TK = 350;
	Comport[3]->ComPanGen(3);

    // Comport[4] - Com5
    Comport[4]->P_Rate = 2;	// evenparity
	Comport[4]->PortName = "COM5";
	Comport[4]->PNL_parent = Form1->Pnl_Com5;
	Comport[4]->ComPanGen(4);

    // Comport[5] - Com6
	Comport[5]->PortName = "COM6";
	Comport[5]->PNL_parent = Form1->Pnl_Com6;
    Comport[5]->B_Rate = CBR_57600;
	Comport[5]->ComPanGen(5);
}
