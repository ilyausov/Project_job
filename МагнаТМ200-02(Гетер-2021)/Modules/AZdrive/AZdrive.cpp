...
void Init_SAZ_drive()
{
	...
	// AZ_drive[0] - MA_POD
	AZ_drive[0]->name = "Подъём п/д"; 	// название для отображения
	AZ_drive[0]->adr = 0x01;
	AZ_drive[0]->Err = 0;
	AZ_drive[0]->Max_err = 5;
	AZ_drive[0]->ACom = 0;
	AZ_drive[0]->Buf_len = 0;
	AZ_drive[0]->diagn_byte = 26;        	// номер байта рабочих диагностик
	...
	AZ_drive[0]->Kom_AZ = &KOM_POD;
	AZ_drive[0]->Otv_AZ = &OTVET_POD;
	AZ_drive[0]->V_AZ = &V_POD;
	AZ_drive[0]->Type_AZ = &TYPE_POD;
	AZ_drive[0]->Spd_AZ = 0;
	AZ_drive[0]->Put_AZ = &PUT_POD;
	AZ_drive[0]->Home_AZ = &HOME_POD;
    AZ_drive[0]->Pr_AZ = &PR_POD;
	AZ_drive[0]->Abs_pos = &TEK_ABS_POD;
	AZ_drive[0]->Otn_pos = &TEK_OTN_POD;
	AZ_drive[0]->SPort = Comport[4];	
