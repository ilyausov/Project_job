...
void InitObjectsRRG()
{
    ...
    {
        ObjRRG[i] = new SRRG();
        ObjRRG[i]->ctVR = 0;				// счетчик выхода на режим
	    ObjRRG[i]->tkVR = 10;				// и его контр. время
	    ObjRRG[i]->ctVRK = 0;				// контрольный счетчик выхода на режим
	    ObjRRG[i]->tkVRK = 3;				// и его контр. время
        ObjRRG[i]->parRRG = 0;				// параметр РРГ

	    ObjRRG[i]->vRRG = 0;				// признак выхода на режим
        ObjRRG[i]->porog1 = 0.1;			// порог появления диагностики "БОР" 10%
	    ObjRRG[i]->porog2 = 0.5;			// порог появления диагностики "НР" 50%
    }
