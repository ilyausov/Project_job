...
void Init_DatMPT200()
{
	for(int i=0;i<DAT_MPT200_COUNT;i++ )
    {
	    ...
	    Dat_MPT200[i]->Err = 0;				// кол-во ошибок
	    Dat_MPT200[i]->Max_err = 5;			// максимум ошибок
	    Dat_MPT200[i]->ACom = 0;				// текущий автоматический запрос
	    Dat_MPT200[i]->RCom = 0;				// текущий ручной апрос
	    Dat_MPT200[i]->Buf_len = 0;			// длина запроса
    }

	// Датчик 1
	Dat_MPT200[0]->name = "Д1(MPT200 шлюз)";            // название для отображения
	Dat_MPT200[0]->adr = "001";			// адрес
	Dat_MPT200[0]->Pres_MPT200 = &D_D1;      // переменная давления для записи
	Dat_MPT200[0]->SPort = Comport[2];     // порт
	Dat_MPT200[0]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MPT200[0]->diagnS_mask = 0x01;		// маска байта связной диагностики
	Dat_MPT200[0]->DatMPT200_Gen();

    // Датчик 2
	Dat_MPT200[1]->name = "Д4(MPT200 масс-спектр.)";            // название для отображения
	Dat_MPT200[1]->adr = "004";			// адрес
	Dat_MPT200[1]->Pres_MPT200 = &D_D4;      // переменная давления для записи
	Dat_MPT200[1]->SPort = Comport[2];     // порт
	Dat_MPT200[1]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MPT200[1]->diagnS_mask = 0x08;		// маска байта связной диагностики
	Dat_MPT200[1]->DatMPT200_Gen();

    // Датчик 3
	Dat_MPT200[2]->name = "Д3(MPT200 камера)";            // название для отображения
	Dat_MPT200[2]->adr = "003";			// адрес
	Dat_MPT200[2]->Pres_MPT200 = &D_D3;      // переменная давления для записи
	Dat_MPT200[2]->SPort = Comport[2];     // порт
	Dat_MPT200[2]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MPT200[2]->diagnS_mask = 0x04;		// маска байта связной диагностики
	Dat_MPT200[2]->DatMPT200_Gen();
}