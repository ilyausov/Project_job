...
#include "drivers\PISODIO.h"    // PISO/PEX-3232
...
int OpenPISO_P32C32(); // попытка связаться с драйвером PISO-P32C32U
...
int OpenPISO_813()
{
     ...
     if(wTotalBoards > 0)
     {
        PISO813_GetConfigAddressSpace( Word(StrToInt(0)), &base_PIO_813,&wIrqNo, &wSubVendor, &wSubDevice, &wSubAux, &wSlotBus, &wSlotDevice);
        PISO813_OutputByte(base_PIO_813, 1);
     }
}
//---------------------------------------------------------------------------
//--ACL7225--//
...
unsigned int PISO_813U( unsigned int* value , int valueCount )
{
    for ( int i = 0 ; i < valueCount ; i++ )
    {
        PISO813_SetChGain(base_PIO_813, i, 0);
        value[i] = DoAverageValue(i,PISO813_AD_Hex(base_PIO_813));
    }
    return 0;
}
//---------------------------------------------------------------------------
