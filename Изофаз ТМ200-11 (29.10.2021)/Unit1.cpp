...
{   SetName();
    ...
    {   // определение нового текущего времени
        ...
        if(timeNew!=timeOld)
        {   for(i=0;i<(timeNew-timeOld);i++) { Synchronize(Timer1ms); }  // запустили диспетчер порта
...
{
	// если установка в режиме отладки запретить чтение Zin
    if ( pr_otl) return;
    ...
    {
        // опросили
        externalError = ISO_813(aik , AIK_COUNT * 8); // DI15 - с 7017 смещение
        // анализ ответа
        switch ( externalError )
        {
            case 0:
    ...
    // OUT ------------------------------------------------
    // записать дискретные выходные сигналы в ISO-P32С32(1)
    ...
    // записать дискретные выходные сигналы в ISO-P32С32(2)
    ...
    // записать дискретные выходные сигналы в ACL7250
    ...
    // запись аналоговых выходных сигналов в ISO-DA16
      ...
      TShiftState Shift)
{
    ...
    if ( Key == kodKlV )
    {
        klGir_gV = 1; // признак нажатия клавиши > "вперед"
    }
    if ( Key == kodKlN )
    {
        klGir_gN = 1; // признак нажатия клавиши < "назад"
    }
}
//---------------------------------------------------------------------------
      ...
      TShiftState Shift)
{
    if ( Key == kodKlP )
    {
        klGir_tV = 0; // признак отжатия клавиши > "вперед"
    }
    if ( Key == kodKlL )
    {
        klGir_tN = 0; // признак отжатия клавиши < "назад"
    }
    if ( Key == kodKlV )
    {
        klGir_gV = 0; // признак нажатия клавиши > "вперед"
    }
    if ( Key == kodKlN )
    {
        klGir_gN = 0; // признак нажатия клавиши < "назад"
    }
}
...
{   if(!ust_ready) return;
    ...
    //VisualDebug();                          //  визуалиазия страницы отладки
    //VisualGraph();                          //  визуализаия графиков
    VisualOperatorDlg();                    //  визуализация диалога оператора

    if(PR_KLASTER && ust_ready) Visual_IM();    // отображение в наладке
}
...
{
Klaster();
//Выводим на передний план активные элементы
vp1->BringToFront();
vp2->BringToFront();
vp3->BringToFront();
vp4->BringToFront();
...
//-------------------------
    ...
    // настройка таблицы параметров автомата
    //Label_Rec -> Caption = IntToStr(ActCell_Rec);
    //Label_Step -> Caption = IntToStr(ActCell_Step);
    
    // настраиваем таблицу рецепта
    //SG_Recept->Cells[0][0] = " Название шага";
    //SG_Recept->Cells[1][0] = " #";
    //SG_Steps->Cells[0][0] = " #";
    //SG_Steps->Cells[1][0] = " Название шага";
    ...
    LibListRefresh();   // обновить лист библиотек

    BtnRDaClick(BtnRDa);

    // загрузка сохраненных настроек нагрева
    ...
    if(FileExists(fName))
    {
        ...
        else{ MessageBox(NULL, "Невозможно загрузить библиотеку", "Ошибка", MB_OK | MB_ICONSTOP); }
    }

    // запуск драйверов плат
    ...
    SetOut(1,1,0x02); // выставляем сигнал "Стоп механихмов"

    LoadGasNames();
    //обновляем название газов
    RenameGases();
    ////////////////////////////////////////////////////////
    //создание элементов "Создание шага"
    ...
    StringGrid1->Cells[1][0]="Название шага";
    LibListRefresh();
...
void TForm1::VisualParam()
{
    ...
    if(PCMain -> ActivePage == TSWork) // Автоматическая мнемосхема
    {
        if(N_ST)
        {
            //Заданные значения
            EdtZadA00 -> Text = FloatToStrF((float)par[N_ST][0]/4095.0*18.0,       ffFixed,5,2);   //Расход РРГ1
            EdtZadA01 -> Text = FloatToStrF((float)par[N_ST][1]/4095.0*9.0,        ffFixed,5,2);   //Расход РРГ2
            EdtZadA02 -> Text = FloatToStrF((float)par[N_ST][2]/4095.0*9.0,        ffFixed,5,2);   //Расход РРГ3
            EdtZadA03 -> Text = FloatToStrF((float)par[N_ST][3]/4095.0*3.6,        ffFixed,5,2);   //Расход РРГ4
            EdtZadA04 -> Text = FloatToStrF((float)par[N_ST][4]/4095.0*9.0,        ffFixed,5,2);   //Расход РРГ5
            EdtZadA05 -> Text = FloatToStrF((float)par[N_ST][5]/4095.0*18.0,       ffFixed,5,2);   //Расход РРГ6
            EdtZadA06 -> Text = FloatToStrF((float)par[N_ST][6]/4095.0*18.0,       ffFixed,5,2);   //Расход РРГ7
            EdtZadA07 -> Text = FloatToStrF((float)par[N_ST][7]/4095.0*CESAR_MAX_IP,    ffFixed,5,0);     //Падающая мощность
            EdtZadA11 -> Text = FloatToStrF((float)par[N_ST][10]/10000*13.3,             ffFixed,   5,1);    //Давление
    ...
    if(PCMain -> ActivePage == TSNalad) //Наладка
    {
        Plast->Visible=false;
        //Заданные значения
        EdtZadA00 -> Text = FloatToStrF((float)par[0][0]/4095.0*18.0,ffFixed,5,2);   //Расход РРГ1
        EdtZadA01 -> Text = FloatToStrF((float)par[0][1]/4095.0*9.0,ffFixed,5,2);   //Расход РРГ2
        EdtZadA02 -> Text = FloatToStrF((float)par[0][2]/4095.0*9.0,ffFixed,5,2);   //Расход РРГ3
        EdtZadA03 -> Text = FloatToStrF((float)par[0][3]/4095.0*3.6,ffFixed,5,2);   //Расход РРГ4
        EdtZadA04 -> Text = FloatToStrF((float)par[0][4]/4095.0*9.0,ffFixed,5,2);   //Расход РРГ5
        EdtZadA05 -> Text = FloatToStrF((float)par[0][5]/4095.0*18.0,ffFixed,5,2);   //Расход РРГ6
        EdtZadA06 -> Text = FloatToStrF((float)par[0][6]/4095.0*18.0,ffFixed,5,2);   //Расход РРГ7
        EdtZadA07 -> Text =  EdtRKon0_7 -> Text;     //Падающая мощность
        EdtZadA11 -> Text = FloatToStrF((float)par[0][10]/10000*13.3,ffFixed,5,1);    //Давление
        EdtZadA09 -> Text = "—";
        EdtZadA10 -> Text = "—";
  ...
  // отображение параметров нагревателей
  for(int i=0;i<PAR_NAGR;i++)
    ...
    // давление баратронов на линейках
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // Температура воды охлаждения форнасоса камеры
  ...
  // Температура воды охлаждения ИП
  ...
  // Температура воды охлаждения ловушки
...
coef_zad->Caption = FloatToStrF(1000.0/(float)nasmod[5],ffFixed,5,0);                      // заданный коэффициент согласования
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if(N_TEK_GIR!=0) { coef_tek->Caption = FloatToStrF(1000.0/(float)N_TEK_GIR,ffFixed,5,0); } // текущий коэффициент согласования
else             { coef_tek->Caption = 0; }

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if((D_D1 >= 0)&&(D_D1 <= 10000)) { davl_D1 -> Caption = FloatToStrF(133.332*pow(10.0,(float)D_D1/1000.0-6.0),ffExponent,3,8); } // показания датчика Д1
if((D_D2 >= 0)&&(D_D2 <= 10000)) { davl_D2 -> Caption = FloatToStrF(133.332*pow(10.0,(float)D_D2/1000.0-6.0),ffExponent,3,8); } // показания датчика Д2
if((D_D3 >= 0)&&(D_D3 <= 10000)) { davl_D3 -> Caption = FloatToStrF((float)D_D3/10000*13.3,                 ffFixed,   5,1); } // показания датчика Д3                                            }
if((D_D4 >= 0)&&(D_D4 <= 10000)) davl_D4 -> Caption=FloatToStrF(100.0*pow(10.0,(float(D_D4)/1000.0-6.8)/0.6),ffExponent,3,8); // показания датчика Д4
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(shr[24])  rrg5 -> Visible = true;    // РРГ5
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(shr[25])  rrg6 -> Visible = true;    // РРГ6
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(shr[26])  rrg7 -> Visible = true;    // РРГ7
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  { case 0x0000: { kl_d4->Picture->Bitmap = rrg_grey->Picture->Bitmap;   break; }; // неопределено
    case 0x4000: { kl_d4->Picture->Bitmap = rrg_white->Picture->Bitmap;  break; }; // открыт
    case 0x8000: { kl_d4->Picture->Bitmap = 0;                           break; }; // закрыт
    case 0xC000: { kl_d4->Picture->Bitmap = rrg_grey->Picture->Bitmap;   break; }; // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(out[4]&0x400)  kl_nap1->Picture->Bitmap = rrg_white->Picture->Bitmap;    // Кл-НАП-1
  else             kl_nap1->Picture->Bitmap = 0;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(out[4]&0x08)  kl_nap2->Picture->Bitmap = rrg_white->Picture->Bitmap;   // Кл-НАП-2
  else             kl_nap2->Picture->Bitmap = 0;
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  switch(zin[0]&0x300)  // ФК-Шл
  { case 0x000: { fk_shl2->Picture->Bitmap = rrg_grey->Picture->Bitmap; break; } // неопределено
    case 0x100: { fk_shl2->Picture->Bitmap = rrg_white->Picture->Bitmap; break; } // открыт
    case 0x200: { fk_shl2->Picture->Bitmap = 0;                             break; } // закрыт
    case 0x300: { fk_shl2->Picture->Bitmap = rrg_grey->Picture->Bitmap; break; } // неоднозначно
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(zin[2]&0x01) { fn_shl->Picture->Bitmap = fn_kam_white->Picture->Bitmap;   } // Форнасос шлюза
  else            { fn_shl->Picture->Bitmap = 0;                          }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(zin[3]&0x20) { datchik1->Visible=true;  } // Есть форвакуум в шлюзе
  else            { datchik1->Visible=false; }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(zin[3]&0x40) { datchik2->Visible=true;  } //Есть форвакуум в ТМН
  ...
  if(PR_KLASTER)
  {
        ...
        {
                case 0x00: { door->Picture->Bitmap = door_grey->Picture->Bitmap; break; } // неопределено
		case 0x100: { door->Picture->Bitmap = 0;                      break; } // открыт
		case 0x200: { door->Picture->Bitmap = door_green->Picture->Bitmap; break; } // закрыт
		case 0x300: { door->Picture->Bitmap = door_grey->Picture->Bitmap; break; } // неоднозначно
        }
  }
  else
  {
	...
	{ case 0x00: { door->Picture->Bitmap = door_grey->Picture->Bitmap; break; } // неопределено
		case 0x04: { door->Picture->Bitmap = 0;                      break; } // открыт
		case 0x08: { door->Picture->Bitmap = door_green->Picture->Bitmap; break; } // закрыт
		case 0x0c: { door->Picture->Bitmap = door_grey->Picture->Bitmap; break; } // неоднозначно
	}
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  switch(zin[1]&0xC00) //Затвор ИП
  { case 0x000: { zatvor->Picture->Bitmap = zatvor_grey->Picture->Bitmap; break; } // неопределено
    case 0x400: { zatvor->Picture->Bitmap = 0;                       break; } // открыт
    case 0x800: { zatvor->Picture->Bitmap = zatvor_green->Picture->Bitmap; break; } // закрыт
    case 0xC00: { zatvor->Picture->Bitmap = zatvor_grey->Picture->Bitmap; break; } // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Форнасос камеры
  if(!(diagn[14]&0x02))//Нет аварии
  {if(zin[2]&0x10) //Нет предупреждения
  {if(zin[2]&0x02){fn_kam->Picture->Bitmap = fn_kam_white->Picture->Bitmap;} //Насос запущен
  else{fn_kam->Picture->Bitmap = 0;}} //Если насос не запущен
  else{fn_kam->Picture->Bitmap = fn_kam_yellow->Picture->Bitmap;}} //Выдаем предупреждение
  else{fn_kam->Picture->Bitmap = fn_kam_red->Picture->Bitmap;} //Выдаем аварию
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
switch(zin[0]&0x3000)  // ФК-ТМН
  { case 0x0000: { fk_tmn->Picture->Bitmap = rrg_grey->Picture->Bitmap;   break; }; // неопределено
    case 0x1000: { fk_tmn->Picture->Bitmap = rrg_white->Picture->Bitmap;  break; }; // открыт
    case 0x2000: { fk_tmn->Picture->Bitmap = 0;                           break; }; // закрыт
    case 0x3000: { fk_tmn->Picture->Bitmap = rrg_grey->Picture->Bitmap;   break; }; // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 switch(zin[1]&0x03)  // Кл-Д2
  { case 0x00: { kl_d2->Picture->Bitmap = rrg_grey->Picture->Bitmap;   break; }; // неопределено
    case 0x01: { kl_d2->Picture->Bitmap = rrg_white->Picture->Bitmap;  break; }; // открыт
    case 0x02: { kl_d2->Picture->Bitmap = 0;                           break; }; // закрыт
    case 0x03: { kl_d2->Picture->Bitmap = rrg_grey->Picture->Bitmap;   break; }; // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
switch(zin[0]&0xC00)  // ФК-Кам
  { case 0x000: { fk_kam->Picture->Bitmap = rrg_grey->Picture->Bitmap; break; }; // неопределено
    case 0x400: { fk_kam->Picture->Bitmap = rrg_white->Picture->Bitmap;break; }; // открыт
    case 0x800: { fk_kam->Picture->Bitmap = 0; break; }; // закрыт
    case 0xC00: { fk_kam->Picture->Bitmap = rrg_grey->Picture->Bitmap; break; }; // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//ТМН
  ...
  else if(zin[2]&0x1400) // разгон/торможение
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
switch(zin[1]&0x300)  // Затвор камеры
  { case 0x000: { zatv->Picture->Bitmap = rrg_v_grey->Picture->Bitmap; break; }; // неопределено
    case 0x100: { zatv->Picture->Bitmap = rrg_v_white->Picture->Bitmap;break; }; // открыт
    case 0x200: { zatv->Picture->Bitmap = 0; break; }; // закрыт
    case 0x300: { zatv->Picture->Bitmap = rrg_v_grey->Picture->Bitmap; break; }; // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(shr[17])
  ...
  {
  switch(zin[1]&0x30) // ДЗ
  { case 0x00: { zasl->Picture->Bitmap = zasl_grey->Picture->Bitmap;    break; } // неопределено
    case 0x10: { zasl->Picture->Bitmap = zasl_white->Picture->Bitmap;   break; } // открыт
    case 0x20: { zasl->Picture->Bitmap = 0;                             break; } // закрыт
    case 0x30: { zasl->Picture->Bitmap = zasl_grey->Picture->Bitmap;    break; } // неоднозначно
  }
  }
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  if(shr[64]||shr[65]) { warm->Visible = true;  } //Нагрев пластины
  else        { warm->Visible = false; }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  switch(zin[1]&0xC0) // Штыри(положение подъемника)
  { case 0x00: { pin_null->Picture->Bitmap = pin_grey ->Picture->Bitmap;                    break; } // неопределено
    case 0x40: { pin_null->Picture->Bitmap = pin_green->Picture->Bitmap; pin_null->Top=554; break; } // подъемник вверху
    case 0x80: { pin_null->Picture->Bitmap = pin_green->Picture->Bitmap; pin_null->Top=572; break; } // подъемник внизу
    case 0xC0: { pin_null->Picture->Bitmap = pin_grey ->Picture->Bitmap;                    break; } // неоднозначно
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
else            fk_shl1->Picture->Bitmap = 0;       //ФК-шл (мягкая откачка)
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if(out[4]&0x800) kl_nap3->Picture->Bitmap = rrg_white->Picture->Bitmap;//КЛ-НАП3
...
void __fastcall TForm1::BtnAutoDaClick(TObject *Sender)
{
    if(((TEdit*)Sender)->Name == "BtnAutoDa")
    {
        ...
        // перносим значения из временного в запомненный массив
...
MemoStat -> Lines -> Add(Label_Time -> Caption + "Переданы наладочные параметры:");
    ...
    if ( EdtRKon0_7 -> Text != EdtRRed0_7 -> Text )
        MemoStat -> Lines -> Add("Мощность ИП: " + EdtRKon0_7 -> Text + " -> " + EdtRRed0_7 -> Text );
    if ( EdtRKon0_10 -> Text != EdtRRed0_10 -> Text )
        MemoStat -> Lines -> Add("Давление: " + EdtRKon0_10 -> Text + " -> " + EdtRRed0_10 -> Text );
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//--Запись значения отлаживаемой переменной--//
...
void TForm1::VisualNasmod()
{
	...
	// 2 - Рабочий вакуум(высокий вакуум)
    EditNastrIn1 -> Text = FloatToStrF(100.0*pow(10,(float(nasmod[1])/1000.0-6.8)/0.6),ffExponent,3,8);
	// 3 - Автосогласование ИП разрещено?
    EditNastrIn2 -> Text = ( nasmod[3] ? "ДА" : "НЕТ" );
	// 4 - Управление скоростью ИП
    EditNastrIn3 -> Text = IntToStr(int(float(nasmod[4]-8192.0-820.0)/205.0+1.5));
	// 5 - Допустимый коэффициент согласования ИП
    EditNastrIn4 -> Text = IntToStr(int(1000.0/float(nasmod[5])+0.5));
	// 6 - Задержка на прокачку камеры после технологического процесса
    EditNastrIn5 -> Text = IntToStr( nasmod[14] );
	// 7 - Уровень откачки камеры после технологического процесса
    EditNastrIn6 -> Text = FloatToStrF(100.0*pow(10,(float(nasmod[7])/1000.0-6.8)/0.6),ffExponent,3,8);
    // 8 - Рабочий цикл с отключением технологического процесса
    EditNastrIn7 -> Text = ( nasmod[8] ? "ДА" : "НЕТ" );
    //Транспортный тест с пластинами
    EditNastrIn17 -> Text = ( nasmod[17] ? "ДА" : "НЕТ" );
    //Технологический процесс с остановкой в начале?
    EditNastrIn18 -> Text = ( nasmod[18] ? "ДА" : "НЕТ" );
}
...
void __fastcall TForm1::BtnNastrDaClick(TObject *Sender)
{
    ...
    // 3 - Автосогласование ИП разрещено?
    EditNastrTo2 -> Text == "ДА" ? nasmod[3] = 1 : nasmod[3] = 0;
    // 4 - Управление скоростью ИП
    ...
    // 8 - Рабочий цикл с отключением технологического процесса
    EditNastrTo7 -> Text == "ДА" ? nasmod[8] = 1 : nasmod[8] = 0;
    //Транспортный тест с пластинами
    EditNastrTo17 -> Text == "ДА" ? nasmod[17] = 1 : nasmod[17] = 0;
    //Технологический процесс с остановкой в начале?
    EditNastrTo18 -> Text == "ДА" ? nasmod[18] = 1 : nasmod[18] = 0;
   MemoStat -> Lines -> Add("------------------------------------------------------------------------------------------------------------------------------------");
    ...
    if ( EditNastrTo1 -> Text != EditNastrIn1 -> Text )
        MemoStat -> Lines -> Add( "Рабочий вакуум(высокий вакуум) : " + EditNastrIn1 -> Text + " -> " + EditNastrTo1 -> Text );
    if ( EditNastrTo2 -> Text != EditNastrIn2 -> Text )
        MemoStat -> Lines -> Add( "Автосогласование ИП разрещено? : " + EditNastrIn2 -> Text + " -> " + EditNastrTo2 -> Text );
    if ( EditNastrTo3 -> Text != EditNastrIn3 -> Text )
        MemoStat -> Lines -> Add( "Управление скоростью ИП : " + EditNastrIn3 -> Text + " -> " + EditNastrTo3 -> Text );
    if ( EditNastrTo4 -> Text != EditNastrIn4 -> Text )
        MemoStat -> Lines -> Add( "Допустимый коэффициент согласования ИП : " + EditNastrIn4 -> Text + " -> " + EditNastrTo4 -> Text );
    if ( EditNastrTo5 -> Text != EditNastrIn5 -> Text )
        MemoStat -> Lines -> Add( "Задержка на прокачку камеры после технологического процесса : " + EditNastrIn5 -> Text + " -> " + EditNastrTo5 -> Text );
    if ( EditNastrTo6 -> Text != EditNastrIn6 -> Text )
        MemoStat -> Lines -> Add( "Уровень откачки камеры после технологического процесса : " + EditNastrIn6 -> Text + " -> " + EditNastrTo6 -> Text );
    if ( EditNastrTo7 -> Text != EditNastrIn7 -> Text )
        MemoStat -> Lines -> Add( "Рабочий цикл с отключением технологического процесса : " + EditNastrIn7 -> Text + " -> " + EditNastrTo7 -> Text );
    ...
    if ( EditNastrTo18 -> Text != EditNastrIn18 -> Text )
        MemoStat -> Lines -> Add( "Технологический процесс с остановкой в начале? : " + EditNastrIn18 -> Text + " -> " + EditNastrTo18 -> Text );
    // закрашивание белым цветом переданных ячекк
...
void TForm1::VisualZagol()
{
    ...
    for(i=0; i<SHR_VALUE_COUNT; i++)
        ...
        {   Form1 -> EditRName -> Text = SHRNames[SHRValue[i]];
            ...
            // в режиме 5 шаг 5 писать шаги режима 4
            ...
            // в режиме 6 шаг 28 писать шаги режима 2
            if((SHRValue[i]==6)&&(shr[SHRValue[i]]==28))
                Form1 -> EditSHRName -> Text = SHR2Names[shr[2]];
            // в режиме 10 шаг 14 писать шаги режима 2
            if((SHRValue[i]==10)&&(shr[SHRValue[i]]==14))
                Form1 -> EditSHRName -> Text = SHR2Names[shr[2]];

            // в режиме 8 шаг 5 писать шаги режима 7
            ...
            if((SHRValue[i]==5)&&(shr[SHRValue[i]]==5))
                Form1 -> EditSHRName -> Text = SHR4Names[shr[4]];
            // в режиме 5 шаг 6 писать шаги режима 6
            if((SHRValue[i]==5)&&(shr[SHRValue[i]]==6))
                Form1 -> EditSHRName -> Text = SHR6Names[shr[6]];
            // в режиме 10 шаг 14 писать шаги режима 2
            if((SHRValue[i]==10)&&(shr[SHRValue[i]]==14))
...
NPanel_String1 -> Caption = "Напуск завершён";
NPanel_String1 -> Visible = true;
NPanel_String2 -> Caption = "Смена кассет разрешена";
NPanel_String2 -> Visible = true;
NPanel_String3 -> Caption = "Шлюз с кассетой?";
...
NPanel_String1 -> Caption = "Откачать шлюз?";
NPanel_String1 -> Visible = true;
...
NPanel_String1 -> Caption = "Начать напуск в шлюз?";
NPanel_String1 -> Visible = true;
NPanel_String2 -> Visible = false;
NPanel_String3 -> Visible = false;
NPanel_DaBut -> Visible = true;
...
NPanel_String1 -> Caption = "Напуск завершен";
NPanel_String1 -> Visible = true;
NPanel_String2 -> Caption = "Выгрузите кассету";
NPanel_String2 -> Visible = true;
APanel_String3 -> Caption = "Откачать шлюз?";
NPanel_String3 -> Visible = true;
NPanel_DaBut -> Visible = true;
...
{
APanel_String1 -> Caption = "Продолжить ";
APanel_String1 -> Visible = true;
APanel_String2 -> Caption = "технологический процесс?";
APanel_String2 -> Visible = true;
APanel_String3 -> Visible = false;
APanel_DaBut -> Visible = true;
APanel_NetBut -> Visible = false;

NPanel_String1 -> Caption = "Продолжить ";
NPanel_String1 -> Visible = true;
NPanel_String2 -> Caption = "технологический процесс?";
NPanel_String2 -> Visible = true;
NPanel_String3 -> Visible = false;
NPanel_DaBut -> Visible = true;
...
void __fastcall TForm1::PanelRCClick(TObject *Sender)
{
	...
	switch(StrToInt(((TPanel*)Sender)->Hint))
	{
		...
		{   LblRejim -> Caption = "Откачка камеры";
            ...
            if(!(zin[2]&0x20))  { ListBoxCondition -> Items -> Add("Нет удал. доступа ФВН камеры"); }
            // есть питания ТМН
            if(!(zin[2]&0x100))  { ListBoxCondition -> Items -> Add("Нет питания ТМН"); }
            // есть удаленный доступ ТМН
            if(!(zin[2]&0x2000))  { ListBoxCondition -> Items -> Add("Нет удал. доступа ТМН"); }
		    // не запущен ни один режим
		    for(i=1;i<40;i++) // не анализируются R_33();
		    { if(i!=33) { if(shr[i]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[i]); } } }
		}; break;
		case 3:
		{   LblRejim -> Caption = "Рабочий цикл (РЦ)";
            ...
            if(!(zin[0]&0x04))  { ListBoxCondition -> Items -> Add("Нет охлаждения ТМН"); }
            // есть охлаждение ИП
		    if(!(zin[0]&0x08)) { ListBoxCondition -> Items -> Add("Нет охлаждения ИП"); }
            // есть охлаждение ловушки
		    if(!(zin[0]&0x10)) { ListBoxCondition -> Items -> Add("Нет охлаждения ловушки"); }
            // есть связь с Д1
		...
		{   LblRejim -> Caption = "Сброс РЦ";
                    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            if((!shr[3])&&(!shr[4])) { ListBoxCondition -> Items -> Add("Не запущен режим Тех. процесс"); }
		    // не запущен режим 5
        ...
        case 9:
		{   LblRejim -> Caption = "Вкл. транспортный тест";
		    // есть давление в пневмосети по zin
		    if(!(zin[0]&0x20)) { ListBoxCondition -> Items -> Add("Нет давления в пневмосети"); }
		    // Затвор открыт
		    if(!(zin[1]&0x04)) { ListBoxCondition -> Items -> Add("Щелевой затвор не открыт"); }
		    // Привод перемещения манипулятора в HOME
		    if(!(zin[4]&0x200)) { ListBoxCondition -> Items -> Add("Привод манипулятора не в HOME"); }
            // не запущен ни один режим
		    for(i=1;i<40;i++) // не анализируются R_33();
		    { if(i!=33) { if(shr[i]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[i]); } } }
		}; break;
		case 109:
		{   LblRejim -> Caption = "Откл. транспортный тест";
		    // запущен режим  9
		...
		{   LblRejim -> Caption = "Закрыть щелевой затвор";
		    ...
		    if(!(zin[4]&0x200)) { ListBoxCondition -> Items -> Add("Привод перемещения манипулятора не в HOME"); }
       		    // не запущен режим 1
		    if(shr[1]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[1]); }
       		    // не запущен режим 2
		    if(shr[2]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[2]); }
        ...
        case 13:
        {   LblRejim -> Caption = "START манипулятора перемещения";
            // Затвор открыт
		    if(!(zin[1]&0x04)) { ListBoxCondition -> Items -> Add("Щелевой затвор не открыт"); }
		    // не запущен режим 1
		    if(shr[1]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[1]); }
		...
		case 29:
		{   LblRejim -> Caption = "Вкл. ВЧГ реактора";
       		    // не запущен режим 1
		    if(shr[1]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[1]); }
       		    // не запущен режим 2
		    if(shr[2]) { ListBoxCondition -> Items -> Add("Запущен режим: " + SHRNames[2]); }
		...
		case 129:
		{   LblRejim -> Caption = "Откл. ВЧГ реактора";
		    // запущен режим 29
		...
		case 33:
		{   LblRejim -> Caption = "Вкл. Нагрев установки";
		    // запущен режим 33
            if(shr[33]) { ListBoxCondition -> Items -> Add("Запущен режим <Нагрев установки>"); }
		}; break;
		case 34:
		{   LblRejim -> Caption = "Откл. Нагрев установки";
		    // запущен режим 33
            if(!shr[33]) { ListBoxCondition -> Items -> Add("Не запущен режим <Нагрев установки>"); }
		}; break;
	...
	{ if(MessageDlg("Подтверждаете запуск режима: " + LblRejim -> Caption + "?", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0) != mrYes ) return; }
...
void __fastcall TForm1::FormClose(TObject *Sender, TCloseAction &Action)
{
    ...
    // запомнили действие в журнал
    MemoStat -> Lines -> Add("------------------------------------------------------------------------------------------------------------------------------------");
    MemoStat -> Lines -> Add( Label_Date -> Caption + " " + Label_Time -> Caption + " : Программа завершена ");
}
//---------------------------------------------------------------------------
void __fastcall TForm1::EdtNRed0Exit(TObject *Sender)
{
    // проверка введенного значения параметров нагрева
...
void __fastcall TForm1::BtnParNDaClick(TObject *Sender)
{
    ...
    if(((TEdit*)Sender)->Name == "BtnParNDa")
    {
        ...
        // убираем выделение введенных ячеек
        ...
        MemoStat -> Lines -> Add(Label_Time -> Caption + "Переданы параметры нагрева");
        VisualParN();

        // сохранение параметров нагрева
        int SizeOfIniFile=(int)sizeof(iniParN);
...
{
    //--Визуализация параметров нагрева
...
void __fastcall TForm1::Prek_1Click(TObject *Sender)
{
   ...
   // сохраняем номер канала и выводим окно ввода
...
void TForm1::Klaster()
{
    ...
    // количество пластин на мнемосхеме
    Plast -> Visible = false; 
...
void __fastcall SComport::Timer_Com2_Timer(TObject *Sender)
{
	...
	if(!PortTask && !Pr_nal)
    {
		if(!PR_KLASTER) PortTask |= 0x01; // Д1 только в кассетном варианте
		PortTask |= 0x0E; // Обновляем автоматическое задание
    }
...
//---------------------------------------------------------------------------
void TForm1::RefreshShTree() //обновление масива и дерева
{
    ///////////////очистка массива///////////////////////////
    Memo_Steps->Clear();
   ...
   for(int i=0;i<Memo_Steps->Lines->Count;i++)
   {
        ...
        klinfo[i].adr=StrToInt(buf[1]);        //запись положения в дереве
        int j;
        for(j=1;j<=21;j++)
            klinfo[i].opcl[j-1]=StrToInt(buf[j+1]); //запись положения клапанов
        for(j=22;buf[j+1]!='@';j++)
            klinfo[i].name+=buf[j+1];                 //запись имени
        j=j+2;
        for(j;j<=buf.Length();j++)
            klinfo[i].hash=klinfo[i].hash+buf[j];
   }
   //////////////добавление в дерево новых элементов////////
   for(int i=0;i<Memo_Steps->Lines->Count;i++)
   {
        AnsiString num ="";
        if(i>=10)
   ...
   TreeView2->FullExpand();//раскрытие ветвей//
...
if(ComboBox1->ItemIndex!=0)
{
    ...
    if (state==0)
    {
        ...
        int i=StrToInt(Edit2->Text);   //номер изменяемого шага
        AnsiString buf;
        buf=ComboBox1->ItemIndex;
        for(int z=0;z<KL_COUNT;z++)   //заполнение состояния клапанов
        {
            buf=buf+Cb_Klapan[z]->Hint;
        }

        buf=buf+Edit1->Text;//заполнение имени шага
        buf=buf+"@"+Now().DateString()+Now().TimeString();
        Memo_Steps->Lines->Strings[i]=buf;
        Memo_Steps->Lines->SaveToFile("libs_1\\Steps.udb");
        buf=ComboBox1->ItemIndex;
        RefreshShTree(); //обновление дерева и масива шагов
        ComboBox1->ItemIndex=StrToInt(buf);
    }
    //новый
    else
...
//---------------------------------------------------------------------------
//изменение текста в названии нового шага//
...
void __fastcall TForm1::AddStepToReceipe(TObject *Sender)
{
    if((Edit2->Text!="")&&(StringGrid1->RowCount-2<N_ST_MAX))
    {
        ...
        AnsiString buf ="";
        buf=        StringGrid1->Cells[0][index]    +   "#";//номер шага
        buf=buf+    klinfo[StrToInt(StringGrid1->Cells[0][index])].hash +   "#";//Хэш шага
        buf=buf+    klinfo[StrToInt(StringGrid1->Cells[0][index])].name +   "#";//Имя шага
        buf=buf+    "0"   +   "#";//расход РРГ1
        buf=buf+    "0"   +   "#";//расход РРГ2
        buf=buf+    "0"   +   "#";//расход РРГ3
        buf=buf+    "0"   +   "#";//расход РРГ4
        buf=buf+    "0"   +   "#";//расход РРГ5
        buf=buf+    "0"   +   "#";//расход РРГ6
        buf=buf+    "0"   +   "#";//расход РРГ7
        buf=buf+    "0"   +   "#";//Мощность ИП
        buf=buf+    "0.5"   +   "#";//положение конденсатора грубо
        buf=buf+    "0.5"   +   "#";//положение конденсатора точно
        buf=buf+    "0"   +   "#";//давление
        buf=buf+    "0"   +   "#";//часы
        buf=buf+    "0"   +   "#";//минуты
        buf=buf+    "0"   +   "#";//секунды
        buf=buf+    "0"   +   "#";//милисекунды
        buf=buf+    "0"   +   "#";//колличество циклов
        ...
        Label_Rec->Caption=" Рецепт : Новый рецепт";
...
void __fastcall TForm1::SaveRecipe(TObject *Sender) //сохранение рецепта//
{
...
//---------------------------------------------------------------------------
 void TForm1::LibListRefresh() // обновить таблицу библиотек
{
    // вывод архивных файлов
    int
        fileCount,
...
{   pr_err=0;
    ...
    for(int i =0;i<Memo_receipt->Lines->Count;i++)
    {
        ...
        if(!err_rec)
            ...
            StringGrid1->Cells[1][i+1]="(ошибка) "+buf1[2];
        StringGrid1->RowCount++;
        if(!err_rec)
        {
            for(int j =0;j<14;j++)
    ...
    if(!pr_err)
    {
        Label_Rec->Caption="Рецепт: "+ListBoxLibrary->Items->Strings[libNmb];
        Label_Rec->Color=0x00400080;
        Label_Rec->Font->Color=clWhite;
    }
    else
    {
        Label_Rec->Caption="Рецепт: (ошибка) "+ListBoxLibrary->Items->Strings[libNmb];
        Label_Rec->Color=0x00400080;
        Label_Rec->Font->Color=clWhite;
    }
    Refresh_kl_info("Clear");
...
{
    if(((TButton*)Sender)->Hint==1)//Выбор рецепта
    {
    ...
    else if(((TButton*)Sender)->Hint==2)//Создание рецепта
    {
...
//---------------------------------------------------------------------------
void TForm1::VisualParA() // отображение параметров автомата
{
    // Перерисовка страницы автомата

    // начинаем с перерисовки рецепта
    ...
    for(int i=0;i<Memo_receipt->Lines->Count;i++)
    {
        if(StringGrid1->Cells[0][i+1]=="000")
            StringGrid1->Cells[1][i+1]="Количество циклов: "+iniLib.par_lib[i][12];
    }

    // отображение переданных параметров
    // расходы РРГ
   ...
   EdtARed13 -> Text = par_temp[ActCell_Rec][13];
    ...
    // отображение библиотечных параметров
    if((pr_lib)&&(!pr_err))
    {
...
void TForm1::Save_parA_temp() // сохранение параметров во временный массив
{
    // расходы РРГ
