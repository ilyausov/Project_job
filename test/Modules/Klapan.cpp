...
void InitObjectsKl()
{
	...
	FvnKam.tkAction[2] = 15;				// к.время задержки после открытия
	...
	FvnShl.tkAction[2] = 10;				// к.время задержки после открытия
	...
	KlShl.zinOut[0].y = 0x1001;			// маска бита упр. выходного сигнала  на "открытие"
	KlShl.zinOut[1].x = 0;				// номер байта упр. выходного сигнала на "закрытие"
	KlShl.zinOut[1].y = 0x1001;			// маска бита упр. выходного сигнала на "закрытие"
	// дискретные входы обратной связи
	...
	KlKam.zinOut[0].y = 0x02;			// маска бита упр. выходного сигнала  на "открытие"
	KlKam.zinOut[1].x = 0;				// номер байта упр. выходного сигнала на "закрытие"
	KlKam.zinOut[1].y = 0x02;			// маска бита упр. выходного сигнала на "закрытие"
	// дискретные входы обратной связи
	...
	// описание Кл-Д5
	KlD5.type = 12;						// тип 1/2
	// нормы объекта
	KlD5.normaNmb[0] = 48;				// код нормы включения
	KlD5.normaNmb[1] = 49;				// код нормы отключения
	// контрольные времена объекта
	KlD5.tkAction[0] = 2;				// к.время открытия
	KlD5.tkAction[1] = 2;				// к.время закрытия
	KlD5.tkAction[2] = 2;				// к.время задержки после открытия
	KlD5.tkAction[3] = 2;				// к.время задержки после закрытия
	// диагностики объекта
	KlD5.diagnObject[0].x = 6;			// номер байта диагностики "не открылся"
	KlD5.diagnObject[0].y = 0x10;		// маска бита диагностики "не открылся"
	KlD5.diagnObject[1].x = 6;			// номер байта диагностики "не закрылся"
	KlD5.diagnObject[1].y = 0x20;		// маска бита диагностики "не закрылся"
	KlD5.diagnObject[2].x = 6;			// номер байта диагностики "не определено"
	KlD5.diagnObject[2].y = 0x40;		// маска бита диагностики "не определено"
	KlD5.diagnObject[3].x = 6;			// номер байта диагностики "неоднозначно"
	KlD5.diagnObject[3].y = 0x80;		// маска бита диагностики "неоднозначно"
	// дискретные управляющие выходы
	KlD5.zinOut[0].x = 0;				// номер байта упр. выходного сигнала на "открытие"
	KlD5.zinOut[0].y = 0x10;			// маска бита упр. выходного сигнала  на "открытие"
	KlD5.zinOut[1].x = 0;				// номер байта упр. выходного сигнала на "закрытие"
	KlD5.zinOut[1].y = 0x10;			// маска бита упр. выходного сигнала на "закрытие"
	// дискретные входы обратной связи
	KlD5.zinIn[0].x = 1;				// номер байта ответного входного сигнала об "открытии"
	KlD5.zinIn[0].y = 0x01;				// маска бита ответного входного сигнала об "открытии"
	KlD5.zinIn[1].x = 1;				// номер байта ответного входного сигнала о "закрытии"
	KlD5.zinIn[1].y = 0x02;				// маска бита ответного входного сигнала о "закрытии"
