	...
	for(int i=0;i<PACKAGE_COUNT;i++)
	{
		if((SPort->PackIn[i]==10)&&(SPort->PackIn[i-1]==13))	// есть конец посылки
		{
			...
			else
			{
                ...
                if(ACom == *ZadCom_DZaslVAT) *Pr_DZaslVAT = true;
                *Otvet_DZaslVAT = ACom;
                return 1;
			}
		}
...
void Init_DZaslVAT()
{
	...
	// Заслонка
	DZaslVAT[0]->name = "Дросс. засл.(VAT)"; // название для отображения

	DZaslVAT[0]->ZadCom_DZaslVAT = &KOM_DZASL;   		// команда задания
	DZaslVAT[0]->ZadData_DZaslVAT = &DATA_DZASL;  		// задание
    DZaslVAT[0]->Otvet_DZaslVAT = &OTVET_DZASL;
    DZaslVAT[0]->Pr_DZaslVAT = &PR_DZASL;
	DZaslVAT[0]->TekDat_DZaslVAT = &D_D3;   			// показания датчика
	DZaslVAT[0]->TekPos_DZaslVAT = &TEK_POZ_DZASL;		// переменная давления для записи
	DZaslVAT[0]->TekDavl_DZaslVAT = &TEK_DAVL_DZASL;	// переменная давления для записи
	...
	DZaslVAT[0]->Err = 0;				// кол-во ошибок
	DZaslVAT[0]->Max_err = 5;			// максимум ошибок
	DZaslVAT[0]->ACom = 0;				// текущий автоматический запрос
	DZaslVAT[0]->RCom = 0;				// текущий ручной апрос
	DZaslVAT[0]->Buf_len = 0;			// длина запроса
