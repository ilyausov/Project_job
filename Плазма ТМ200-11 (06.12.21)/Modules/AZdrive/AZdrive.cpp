...
void Init_SAZ_drive()
{
	...
	// AZ_drive[0] - MA_per
	AZ_drive[0]->name = "Перем. манип."; 	// название для отображения
	AZ_drive[0]->adr = 0x01;
	AZ_drive[0]->Err = 0;
	AZ_drive[0]->Max_err = 5;
	AZ_drive[0]->ACom = 0;
	AZ_drive[0]->Buf_len = 0;
	AZ_drive[0]->diagn_byte = 5;        	// номер байта рабочих диагностик
	AZ_drive[0]->diagnS_byte = 2;        	// номер байта связной диагностики
	AZ_drive[0]->diagnS_mask = 0x01;		// маска байта связной диагностики
	
	AZ_drive[0]->CT_AZ = &CT_PER;				// счетчик механизма
	AZ_drive[0]->home_norm = 43;
	AZ_drive[0]->norm = 44;
	AZ_drive[0]->Stop_byte = 2;			// бит Стоп мех (общий)
	AZ_drive[0]->Stop_mask = 0x40;	
	AZ_drive[0]->Rdy_byte = 4;			// бит готовности
	AZ_drive[0]->Rdy_mask = 0x100;	
	AZ_drive[0]->Run_byte = 1;			// бит движения
	AZ_drive[0]->Run_mask = 0x01;	
	AZ_drive[0]->HomeOut_byte = 1;		// бит движения в Home
	AZ_drive[0]->HomeOut_mask = 0x02;	
	AZ_drive[0]->HomeIn_byte = 4;			// бит нахождения в Home
	AZ_drive[0]->HomeIn_mask = 0x200;

    AZ_drive[0]->Zad_type = 0;
    AZ_drive[0]->Zad_pos = 0;
    AZ_drive[0]->Zad_spd = 0;
    AZ_drive[0]->Tek_pos = 0;
	
	AZ_drive[0]->Kom_AZ = &KOM_PER;
	AZ_drive[0]->Otv_AZ = &OTVET_PER;
	AZ_drive[0]->V_AZ = &V_PER;
	AZ_drive[0]->Type_AZ = &TYPE_PER;
	AZ_drive[0]->Spd_AZ = 0;
	AZ_drive[0]->Put_AZ = &PUT_PER;
	AZ_drive[0]->Home_AZ = &HOME_PER;
    AZ_drive[0]->Pr_AZ = &PR_PER;    
	AZ_drive[0]->Abs_pos = &TEK_ABS_PER;
	AZ_drive[0]->Otn_pos = &TEK_OTN_PER;
	AZ_drive[0]->SPort = Comport[2];	
	AZ_drive[0]->AZdrive_Gen();

	// AZ_drive[1] - MA_pov
	AZ_drive[1]->name = "Поворот манип."; 	// название для отображения
	AZ_drive[1]->adr = 0x02;
	AZ_drive[1]->Err = 0;
	AZ_drive[1]->Max_err = 5;
	AZ_drive[1]->ACom = 0;
	AZ_drive[1]->Buf_len = 0;
	AZ_drive[1]->diagn_byte = 26;        	// номер байта рабочих диагностик
	AZ_drive[1]->diagnS_byte = 2;        	// номер байта связной диагностики
	AZ_drive[1]->diagnS_mask = 0x02;		// маска байта связной диагностики
	
	AZ_drive[1]->CT_AZ = &CT_POV;				// счетчик механизма
	AZ_drive[1]->home_norm = 66;
	AZ_drive[1]->norm = 67;
	AZ_drive[1]->Stop_byte = 2;			// бит Стоп мех (общий)
	AZ_drive[1]->Stop_mask = 0x40;	
	AZ_drive[1]->Rdy_byte = 4;			// бит готовности
	AZ_drive[1]->Rdy_mask = 0x800;	
	AZ_drive[1]->Run_byte = 1;			// бит движения
	AZ_drive[1]->Run_mask = 0x04;	
	AZ_drive[1]->HomeOut_byte = 1;		// бит движения в Home
	AZ_drive[1]->HomeOut_mask = 0x08;	
	AZ_drive[1]->HomeIn_byte = 4;			// бит нахождения в Home
	AZ_drive[1]->HomeIn_mask = 0x1000;

    AZ_drive[1]->Zad_type = 0;
    AZ_drive[1]->Zad_pos = 0;
    AZ_drive[1]->Zad_spd = 0;
    AZ_drive[1]->Tek_pos = 0;
	
	AZ_drive[1]->Kom_AZ = &KOM_POV;
	AZ_drive[1]->Otv_AZ = &OTVET_POV;
	AZ_drive[1]->V_AZ = &V_POV;
	AZ_drive[1]->Type_AZ = &TYPE_POV;
	AZ_drive[1]->Spd_AZ = 0;
	AZ_drive[1]->Put_AZ = &PUT_POV;
	AZ_drive[1]->Home_AZ = &HOME_POV;
    AZ_drive[1]->Pr_AZ = &PR_POV;    
	AZ_drive[1]->Abs_pos = &TEK_ABS_POV;
	AZ_drive[1]->Otn_pos = &TEK_OTN_POV;
	AZ_drive[1]->SPort = Comport[2];	
	AZ_drive[1]->AZdrive_Gen();

	// AZ_drive[2] - Kas
	AZ_drive[2]->name = "Перем. кассеты"; 	// название для отображения
	AZ_drive[2]->adr = 0x03;
	AZ_drive[2]->Err = 0;
	AZ_drive[2]->Max_err = 5;
	AZ_drive[2]->ACom = 0;
	AZ_drive[2]->Buf_len = 0;
	AZ_drive[2]->diagn_byte = 27;        	// номер байта рабочих диагностик
	AZ_drive[2]->diagnS_byte = 2;        	// номер байта связной диагностики
	AZ_drive[2]->diagnS_mask = 0x04;		// маска байта связной диагностики
	
	AZ_drive[2]->CT_AZ = &CT_KAS;				// счетчик механизма
	AZ_drive[2]->home_norm = 70;
	AZ_drive[2]->norm = 71;
	AZ_drive[2]->Stop_byte = 2;			// бит Стоп мех (общий)
	AZ_drive[2]->Stop_mask = 0x40;	
	AZ_drive[2]->Rdy_byte = 4;			// бит готовности
	AZ_drive[2]->Rdy_mask = 0x01;	
	AZ_drive[2]->Run_byte = 1;			// бит движения
	AZ_drive[2]->Run_mask = 0x10;	
	AZ_drive[2]->HomeOut_byte = 1;		// бит движения в Home
	AZ_drive[2]->HomeOut_mask = 0x20;	
	AZ_drive[2]->HomeIn_byte = 4;			// бит нахождения в Home
	AZ_drive[2]->HomeIn_mask = 0x02;

    AZ_drive[2]->Zad_type = 0;
    AZ_drive[2]->Zad_pos = 0;
    AZ_drive[2]->Zad_spd = 0;
    AZ_drive[2]->Tek_pos = 0;
	
	AZ_drive[2]->Kom_AZ = &KOM_KAS;
	AZ_drive[2]->Otv_AZ = &OTVET_KAS;
	AZ_drive[2]->V_AZ = &V_KAS;
	AZ_drive[2]->Type_AZ = &TYPE_KAS;	
	AZ_drive[2]->Spd_AZ = 0;
    AZ_drive[2]->Put_AZ = &PUT_KAS;
	AZ_drive[2]->Home_AZ = &HOME_KAS;
    AZ_drive[2]->Pr_AZ = &PR_KAS;
	AZ_drive[2]->Abs_pos = &TEK_ABS_KAS;
	AZ_drive[2]->Otn_pos = &TEK_OTN_KAS;
	AZ_drive[2]->SPort = Comport[2];
