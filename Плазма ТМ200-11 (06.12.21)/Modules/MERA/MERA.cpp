...
//---------------------------------------------------------------------------
double SMERA_dat::raspakMERA()				// распаковка ответа
{
	char strB1[] = {0,0,0};
	char strB2[] = {0,0,0};
	char strB3[] = {0,0,0};
...
void Init_DatMERA()
{
	...
	// Датчик 1
	Dat_MERA[0]->name = "Д1-Шлюз(ПМТ)";            // название для отображения
	Dat_MERA[0]->adr = "01";			// адрес
	Dat_MERA[0]->Pres_MERA = &D_D1;      // переменная давления для записи
	Dat_MERA[0]->SPort = Comport[1];     // порт
	Dat_MERA[0]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MERA[0]->diagnS_mask = 0x01;		// маска байта связной диагностики

	Dat_MERA[0]->Err = 0;				// кол-во ошибок
	Dat_MERA[0]->Max_err = 5;			// максимум ошибок
	Dat_MERA[0]->ACom = 0;				// текущий автоматический запрос
	//Dat_MERA[0]->RCom = 0;				// текущий ручной апрос
	Dat_MERA[0]->Buf_len = 0;			// длина запроса
	Dat_MERA[0]->DatMERA_Gen();
	
	// Датчик 2
	Dat_MERA[1]->name = "Д5-ТМН(ПМТ)";            // название для отображения
	Dat_MERA[1]->adr = "05";			// адрес
	Dat_MERA[1]->Pres_MERA = &D_D5;      // переменная давления для записи
	Dat_MERA[1]->SPort = Comport[1];     // порт
	Dat_MERA[1]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MERA[1]->diagnS_mask = 0x10;		// маска байта связной диагностики

	Dat_MERA[1]->Err = 0;				// кол-во ошибок
	Dat_MERA[1]->Max_err = 5;			// максимум ошибок
	Dat_MERA[1]->ACom = 0;				// текущий автоматический запрос
	//Dat_MERA[1]->RCom = 0;				// текущий ручной апрос
	Dat_MERA[1]->Buf_len = 0;			// длина запроса
	Dat_MERA[1]->DatMERA_Gen();
	
	// Датчик 3
	Dat_MERA[2]->name = "Д2-П/д(ПМТ)";            // название для отображения
	Dat_MERA[2]->adr = "02";			// адрес
	Dat_MERA[2]->Pres_MERA = &D_D2;      // переменная давления для записи
	Dat_MERA[2]->SPort = Comport[1];     // порт
	Dat_MERA[2]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MERA[2]->diagnS_mask = 0x02;		// маска байта связной диагностики

	Dat_MERA[2]->Err = 0;				// кол-во ошибок
	Dat_MERA[2]->Max_err = 5;			// максимум ошибок
	Dat_MERA[2]->ACom = 0;				// текущий автоматический запрос
	//Dat_MERA[2]->RCom = 0;				// текущий ручной апрос
	Dat_MERA[2]->Buf_len = 0;			// длина запроса
