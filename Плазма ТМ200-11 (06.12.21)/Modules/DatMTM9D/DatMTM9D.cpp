...
void Init_DatMTM9D()
{
	...
	// Датчик 1
	Dat_MTM9D[0]->name = "Д4(MTM9D)";            // название для отображения
	Dat_MTM9D[0]->adr = "001";			// адрес
	Dat_MTM9D[0]->Pres_DMTM9D = &D_D4;      // переменная давления для записи
	Dat_MTM9D[0]->SPort = Comport[3];     // порт
	Dat_MTM9D[0]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MTM9D[0]->diagnS_mask = 0x08;		// маска байта связной диагностики

	Dat_MTM9D[0]->Err = 0;				// кол-во ошибок
	Dat_MTM9D[0]->Max_err = 5;			// максимум ошибок
	Dat_MTM9D[0]->ACom = 0;				// текущий автоматический запрос
	Dat_MTM9D[0]->RCom = 0;				// текущий ручной апрос
	Dat_MTM9D[0]->Buf_len = 0;			// длина запроса
