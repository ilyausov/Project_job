...
void Init_DatMTM9D()
{
	for(int i=0;i<DAT_MTM9D_COUNT;i++ )
	{
		...
		Dat_MTM9D[i]->Err = 0;				// кол-во ошибок
		Dat_MTM9D[i]->Max_err = 5;			// максимум ошибок
		Dat_MTM9D[i]->ACom = 0;				// текущий автоматический запрос
		Dat_MTM9D[i]->RCom = 0;				// текущий ручной апрос
		Dat_MTM9D[i]->Buf_len = 0;			// длина запроса
	}

	// Датчик 1
	Dat_MTM9D[0]->name = "Д1(MTM9D)";            // название для отображения
	Dat_MTM9D[0]->adr = "001";			// адрес
	Dat_MTM9D[0]->Pres_DMTM9D = &D_D1;      // переменная давления для записи
	Dat_MTM9D[0]->SPort = Comport[1];     // порт
	Dat_MTM9D[0]->diagnS_byte = 0;        // номер байта связной диагностики
	Dat_MTM9D[0]->diagnS_mask = 0x01;		// маска байта связной диагностики
