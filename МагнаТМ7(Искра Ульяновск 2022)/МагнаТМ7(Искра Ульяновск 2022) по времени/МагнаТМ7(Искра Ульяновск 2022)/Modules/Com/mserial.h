// Класс для работы с компортом по RS-232 в асинхронном режиме
...
// Константы тайм-аутов в миллисекундах
#define    TimeoutInterval    10
              // максимальный интервал между двумя символами
#define    TimeoutChar        1
             // максимальное время на принятие одного символа
#define    TimeOutWait        150
             // максимальное время ожидания посылки
...
class MSerial
{
   public:
      ...
      boolean Open(const char port[], DWORD BaudRate, BYTE ByteSize,
      ...
      // возвращает число принятых символов в буфере приема
...
boolean MSerial::Open(const char port[], DWORD BaudRate, BYTE ByteSize,
                                                   BYTE Parity, BYTE StopBits)
{
    handle = CreateFile
                    (
